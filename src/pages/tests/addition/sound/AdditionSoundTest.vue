...
import type {
  ...
  SoundHardTestQuestionDto,
  SoundHardTestSumType,
} from './AdditionSoundTest.types.ts';
...
<template>
  <div class="wrapper">
    <div class="greetings block" v-if="step == 0">
      <h1 class="heading">
        Тест "Оценка скорости реакции на сложный звуковой сигнал - сложение в
        уме"
      </h1>
      <div class="description">
        <p>
          Тест генерирует случайные суммы двух чисел, и Ваша задача как можно
          быстрее определить чётность суммы
        </p>
        <ul>
    ...
    <div
      class="test block"
      v-if="step == 1 && questions.length > questionNumber"
    >
      <div class="stats">
        <h4 class="score">Score: {{ score }} / {{ questionsCount }}</h4>
        <h4 class="question">
          Question: {{ questionNumber + 1 }} / {{ questionsCount }}
        </h4>
      </div>
      <div class="controls">
        <CommonButton
    ...
    <div class="error block" v-if="step == -1">
      ...
      <div class="description">
        ...
        <p>
          Если ни один из вариантов Вам не подходит, то можете связаться с нами
        </p>
      </div>
      <div class="controls">
