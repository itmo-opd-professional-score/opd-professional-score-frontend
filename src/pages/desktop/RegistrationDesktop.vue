...
export default defineComponent({
  ...
  methods: {
    validatePassword() {
      if (this.password == "") {
        ...
        eventBus.emit("registerFirstStep", {email: this.email, username: this.username})
      }
    },
    async sendCode() {
  ...
  mounted() {
    eventBus.on("approveCode", (code: string) => eventBus.emit("registerSecondStep", {
      email: this.email,
      username: this.username,
